...
def render_vocab_practice(student_code: str, level: str) -> None:
    ...
    if completed_words:
        ...
        preview_count = 6 if len(completed_words) > 6 else len(completed_words)
        preview = ", ".join(f"`{word}`" for word in completed_words[:preview_count])
        if preview:
            suffix = " …" if len(completed_words) > preview_count else ""
            st.caption(f"Recently practiced: {preview}{suffix}")
        ...
        if last_session:
            ...
            if practiced:
                st.info(
                    "Last session: " + ", ".join(f"`{word}`" for word in practiced),
                    icon="🕒",
                )

        with st.expander("See practiced words", expanded=True):
